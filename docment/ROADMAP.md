...
**Pattern matching — first slice (0.3.2 scope, in progress):** `as v` binding
on `when` EnumVariant arms landed `e7bf778`. Whole-scrutinee binding (enums are
tag-only). Full-stack: lexer, parser, semantic, IR, interpreter. 10 test
fixtures (2 positive, 8 rejection). Guard clauses are the next 0.3.2 piece.
...
**2026-07-05:** Handle.drop (D2.5c, `3ea986d`) closed the scalar Handle arc
on submain — generational safety and double-drop non-aliasing empirically
proven. `as v` binding on `when` EnumVariant arms (`e7bf778`) opened 0.3.2
pattern matching work. Roadmap reconciliation (`10ac2ed`) formally recorded
the version sequence for the first time. Matrix: 292/0 on main, ~310/0 on
submain. Submain 6 commits ahead of main.
