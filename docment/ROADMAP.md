...
- [ ] Phase 15 — Cranelift JIT 0.1 target (CX-74 exit-code propagation merged; print arg widening 08fa2f9; literal-width narrowing complete across 5 operator sites; CX-57/58/60/63/64/66 instruction coverage in flight; 222 PASS / 64 SKIP / 0 PARITY_FAIL across 286 fixtures (submain); main frozen at 230/230 matrix)
...
**2026-06-24:** D2.3c + D2.3d landed on submain. D2.3c: literal string concat folds at compile time, content equality via cx_str_eq host callback (parity 215→220). D2.3d: print-time string interpolation decomposes print("a {x} b") into inline print sequence (no new string built), f64 print via cx_print_f64_inline (parity 220→222). Static string subset declared complete — only R6-deferred dynamic strings remain. Submain 22 commits ahead of main. Matrix 230/0 on main, 286/0 on submain.

**2026-06-23:** D2.3b landed on submain — first string value flowing through IR. Static string literals lower to leaked {ptr, len} descriptors; cx_print_str host callback prints string values byte-for-byte matching the interpreter. Parity 215/71/0 across 286 fixtures. String concatenation, comparison, and interpolation still SKIP (D2.3c/d). Submain 20 commits ahead of main. Matrix 230/0 on main.
