...
//   ──────────────────┼───────────────────┼──────────────────────────────────
//   I/O (stdout)      | print, println    | LOWERABLE — see lower_printn_stmt (CX-137)
            ...
            if let SemanticExprKind::Call { callee, args, .. } = &expr.kind {
                match callee.as_str() {
                    ...
                    "print" | "println" | "printn" => {
                        return lower_printn_stmt(callee.as_str(), args, ctx, current)
                    }
                    _ => {}
                }
            }
...
/// Lower `print(n)`, `println(n)`, or `printn(n)` to
/// `IrInst::Call { callee: "cx_printn", args: [i64_value] }`.
///
/// All three Cx print builtins lower identically: they call the `cx_printn`
/// runtime intrinsic, which is pre-declared in the JIT module as an imported
/// C-ABI symbol.  The name difference is preserved at the source level but
/// collapsed at IR level because all three produce a newline-terminated integer
/// print in the JIT.
///
...
fn lower_printn_stmt(
    ...
    if arg.ty != IrType::I64 {
        return Err(LoweringError::UnsupportedSemanticConstruct {
            construct: format!(
                "{} argument must be I64, got {:?} — other types not yet supported",
                builtin_name, arg.ty
            ),
        });
    }
...
mod tests {
    ...
    #[test]
    fn print_lowers_to_cx_printn_call() {
        ...
        let cx_printn_calls: Vec<_> = main_fn
            ...
            .collect();
        assert_eq!(cx_printn_calls.len(), 1, "print should emit exactly one cx_printn call");
    ...
    #[test]
    fn println_lowers_to_cx_printn_call() {
        // println(99i64) must lower to IrInst::Call{callee:"cx_printn"} and pass validation.
        let program = SemanticProgram {
            stmts: vec![builtin_stmt(
                "println",
                vec![SemanticCallArg::Expr(int_expr(99, SemanticType::I64))],
            )],
            enums: vec![],
        };
        let module = lower_and_validate(&program);
        ...
        let cx_printn_calls: Vec<_> = main_fn
            ...
            .filter(|inst| matches!(inst, IrInst::Call { callee, .. } if callee == "cx_printn"))
            .collect();
        assert_eq!(cx_printn_calls.len(), 1, "println should emit exactly one cx_printn call");
        match cx_printn_calls[0] {
            IrInst::Call { dst, callee, args, return_ty } => {
                assert!(dst.is_none(), "cx_printn is void — no destination");
                assert_eq!(callee, "cx_printn");
